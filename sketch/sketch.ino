...
const int relayPin = D6; // ✅ Tambahkan pin relay di sini
  ...
  if (millis() - lastCheck > 10000) {  
    ...
    while (numNew) {  
      for (int i = 0; i < numNew; i++) {  
        ...
        } else if (text == "/relay_on") {  // ✅ Telegram relay ON
          digitalWrite(relayPin, HIGH);  
          showTempMessage("Relay On");  
          bot.sendMessage(chatId, "✅ Relay dinyalakan", "");  
        } else if (text == "/relay_off") {  // ✅ Telegram relay OFF
...
void setup() {  
  ...
  server.on("/toggleRelay", []() {  // ✅ Endpoint toggleRelay
    if (server.hasArg("state")) {
      String state = server.arg("state");
